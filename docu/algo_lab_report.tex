...
%% - time in ms with 3 floating numbers √
...
%% - better pseudocode (see listings package)
...
\begin{description}
	...
	\caption{Optimal minimum search}\label{opminsearch}
		...
		\Procedure{OptimalMinSearch}{$I, s, e$}
			...
			\State \State $mIdx \gets s$
			\For{$i \gets (s+1)~\algorithmicto~e$}
				\If{$I[i] < I[mIdx]$} \State $m \gets i$ \EndIf
			\EndFor
			\State \Return $mIdx$
			\EndProcedure
		\end{algorithmic}
...
\begin{description}
	...
	\caption{Merge Part 1 - Write into $T$}
		...
		\end{algorithmic}
	\end{algorithm}	
	The second part for writing back into the array:
	\begin{algorithm}
